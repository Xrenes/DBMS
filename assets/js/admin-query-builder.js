...
const CATEGORIES = {
  ...
  'Evaluation': ['evaluation_forms','evaluation_responses'],
  'Faculty Management': ['faculty_profiles','faculty_leave_requests'],
  'Registration': ['registration_requests','registration_items'],
  'Clearance': ['clearance_requests','clearance_steps'],
  'Admin / Monitoring': ['audit_logs','ledger_events','system_config','fact_academic','saved_queries','query_operations'],
};
...
async function selectSrc(name, type) {
  ...
  // Fetch DDL for the table/view
  try {
    const ddlRes = await api(`/query-builder/tables/${encodeURIComponent(name)}/ddl`);
    currentDdl = ddlRes.data || '';
  } catch(e) { currentDdl = ''; }
  renderOps();
  renderCatalog();
  // Show DDL in View SQL immediately
  updateSqlPreview();
  execQuery();
...
function highlightSql(sql) {
  const kws = ['SELECT','FROM','WHERE','JOIN','INNER','LEFT','RIGHT','FULL','OUTER','CROSS','ON','AND','OR','NOT','IN','LIKE','IS','NULL','AS','ORDER','BY','GROUP','HAVING','LIMIT','OFFSET','UNION','ALL','DISTINCT','COUNT','SUM','AVG','MIN','MAX','DESC','ASC','BETWEEN','EXISTS','CASE','WHEN','THEN','ELSE','END','CREATE','VIEW','TABLE','INDEX','EXPLAIN','SHOW','DESCRIBE','INSERT','INTO','VALUES','UPDATE','SET','DELETE','CALL'];
...
function getSqlForPreview() {
  // If only select_source op exists (no other operations), show DDL
  const hasOps = ops.some(o => o.type !== 'select_source');
  if (!hasOps && currentDdl) return currentDdl;
  // Otherwise show the generated query
  return buildSql();
}

function updateSqlPreview() {
  const sql = getSqlForPreview();
  const hasOps = ops.some(o => o.type !== 'select_source');
  // Update the label to indicate DDL vs Query
  const label = document.querySelector('#sqlBar .sql-head span');
  if (label) label.textContent = (!hasOps && currentDdl) ? 'Table Structure (DDL)' : 'Generated SQL';
  document.getElementById('sqlCode').innerHTML = highlightSql(sql || '-- Select a table to begin');
  // Do NOT auto-open View SQL panel — let user click the button
}
...
function toggleSql() {
  ...
  if (bar.classList.contains('open')) {
    ...
    bar.style.height = '';
    // Update preview content and label when opening
    const hasOps = ops.some(o => o.type !== 'select_source');
    const label = document.querySelector('#sqlBar .sql-head span');
    if (label) label.textContent = (!hasOps && currentDdl) ? 'Table Structure (DDL)' : 'Generated SQL';
    const sql = getSqlForPreview();
...
function copySql() { navigator.clipboard.writeText(getSqlForPreview()).then(()=>toast('SQL copied!','inf')); }
...
async function execQuery() {
  ...
  // Determine if DML (INSERT/UPDATE/DELETE/CALL)
  const trimmed = sql.trim().toUpperCase();
  const isDml = ['INSERT','UPDATE','DELETE','CALL'].some(kw => trimmed.startsWith(kw));
  const endpoint = isDml ? '/query-builder/execute-dml' : '/query-builder/execute';

  try {
    const t0 = performance.now();
    const r = await api(endpoint,{method:'POST',body:JSON.stringify({sql})});
    const elapsed = Math.round(performance.now()-t0);
    console.log('execQuery result:', r.success, 'rows:', r.data?.length);
    if (r.success) {
      if (isDml) {
        const msg = r.message || `${r.meta?.affectedRows||0} row(s) affected`;
        resultData = [];
        resultMeta = r.meta||{};
        renderGrid();
        setStatus('ok', msg, r.meta?.affectedRows);
        toast(msg, 'ok');
      ...
      document.getElementById('topStatusText').textContent = isDml
        ? (r.message || 'Done')
        : `${resultData.length} rows in ${r.meta?.executionTime||elapsed+'ms'}`;
    } else {
...
// Clear source — go back to table list
function clearSource() {
  ...
  updateSqlPreview();
  renderGrid();
  // Close View SQL and Write SQL panels
  const sqlBar = document.getElementById('sqlBar');
  sqlBar.classList.remove('open');
  sqlBar.style.height = '0';
...
function editOp(id) {
  ...
  if (!op) return;
  switch(op.type) {
    case 'select_source': openConfigModal('modalSource', op); break;
...
const TEMPLATES = {
  ...
  results: `SELECT s.student_code, u.full_name, c.course_code, c.title, r.total_mark, r.grade_code, gs.grade_point, sem.name AS semester\nFROM results r\nJOIN enrollments e ON r.enrollment_id = e.enrollment_id\nJOIN students s ON e.student_id = s.student_id\nJOIN users u ON s.user_id = u.user_id\nJOIN course_offerings co ON e.offering_id = co.offering_id\nJOIN courses c ON co.course_id = c.course_id\nJOIN semesters sem ON co.semester_id = sem.semester_id\nLEFT JOIN grade_scale gs ON r.grade_code = gs.grade_code\nLIMIT 50;`,
  ...
  evaluation: `SELECT * FROM vw_evaluation_summary;`,
  faculty_leave: `SELECT fl.leave_id, u.full_name AS faculty_name, fp.designation, fl.leave_type, fl.start_date, fl.end_date,\n  DATEDIFF(fl.end_date, fl.start_date)+1 AS days, fl.reason, fl.status,\n  a.full_name AS approved_by\nFROM faculty_leave_requests fl\nJOIN users u ON fl.faculty_user_id = u.user_id\nJOIN faculty_profiles fp ON fp.user_id = fl.faculty_user_id\nLEFT JOIN users a ON fl.approved_by = a.user_id\nORDER BY fl.created_at DESC;`,
  registration: `SELECT * FROM vw_registration_overview;`,
  clearance: `SELECT * FROM vw_clearance_tracking;`,
  admit_student: `-- Admit a new student (creates user + role + student record + ledger entry)\n-- Password is auto-encrypted by the server (bcrypt)\nCALL sp_admit_student(\n  'rahim.uddin@diu.edu.bd',   -- email\n  'Rahim Uddin',              -- full_name\n  '+8801700000099',            -- phone\n  'student123',                -- password (plain text, auto-encrypted)\n  '221-15-6001',              -- student_code (DIU format)\n  1,                           -- program_id (1=BSc CSE)\n  2026,                        -- batch_year\n  'A'                          -- section\n);`,
  add_faculty: `-- Add a new faculty member\nINSERT INTO users (email, password_hash, full_name, phone, status)\nVALUES ('new.faculty@diu.edu.bd', 'faculty123', 'Dr. Faculty Name', '+8801800000000', 'active');\n\nINSERT INTO user_roles (user_id, role_id) VALUES (LAST_INSERT_ID(), 3);\n\nINSERT INTO faculty_profiles (user_id, dept_id, designation, specialization, joining_date, employment_type)\nVALUES (LAST_INSERT_ID(), 1, 'Lecturer', 'Software Engineering', CURDATE(), 'full-time');`,
  report_card: `-- Semester Report Card with CURSOR + stored functions\nCALL sp_semester_report_card(7);`,
  fee_report: `-- Fee Collection Summary with CURSOR\nCALL sp_fee_summary(7);`,
  test_functions: `-- Test all stored functions\nSELECT\n  fn_calculate_sgpa(1, 7) AS sgpa_student1,\n  fn_calculate_cgpa(1) AS cgpa_student1,\n  fn_academic_standing(fn_calculate_cgpa(1)) AS standing,\n  fn_payment_percentage(1) AS inv1_payment_pct,\n  fn_attendance_pct(1, 1) AS attendance_pct;`
};
