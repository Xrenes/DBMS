...
router.post('/login', async (req, res, next) => {
  try {
    // Accept email, studentId, or both (portal login pages send studentId)
    ...
    // Look up user by email, student_code, or faculty email
    ...
    // If not found by email, try student_code
    if (users.length === 0) {
      users = await query(
        `SELECT u.user_id, u.email, u.password_hash, u.full_name, u.status,
                GROUP_CONCAT(DISTINCT r.role_name) as roles
