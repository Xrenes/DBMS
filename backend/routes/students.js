...
// Update student profile (current logged-in student)
router.put('/profile', authenticate, async (req, res, next) => {
  try {
    ...
    if (phone !== undefined) {
      await query('UPDATE users SET phone = ?, updated_at = NOW() WHERE user_id = ?', [phone, userId]);
    }
    ...
    await query(
      `INSERT INTO audit_logs (actor_user_id, action, table_name, record_pk, new_row)
       VALUES (?, 'UPDATE', 'users', ?, ?)`,
      [userId, String(userId), JSON.stringify({ phone, address })]
    );
